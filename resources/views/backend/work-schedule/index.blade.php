    ...
    <section class="content overlay">
        ...
        <div class="box">
            <div class="box-body no-padding">
                ...
                @include('backend.work-schedule.list-table')
            </div>
        </div>
    </section>
@stop
...
<script>
    !function ($) {
        var url_href = '{{ route("admin.workschedule.index") }}';
        ...
        $.ajax({
            ...
            success:function(data) {
                ...
                if (data.errors) {
                    ...
                    if (data.errors.time_in1111 || data.errors.tine_out) {
                        $('#time-in-out').html(data.errors.time_in1111[0]);
                    } else {
                        $('#time-in-out').html('');
                    }
    ...
    $('.open-modal').on('click', function () {
        ...
        $.get(get_url, function (response) {
            if (response.data.department.type == 1) {
                $('.office').show();
            }
            $.each(response.data, function (key, value) {
                ...
                if (key == 'category_shift_id') {
                    $('#category_shift_id').val(response.data.category_shift_id);
                    $("#category_shift_id").attr('selected', 'selected').change();
                }
                if (key == 'type' && value == 1) {
    ...
    !function ($) {
        $(function () {
            ...
            table.columns('.company_id').every(function () {
                ...
                callSelect2AutoWidth('.company_select')
            });
            table.columns('.department_id').every(function () {
                ...
                callSelect2AutoWidth('.department_select')
            });
