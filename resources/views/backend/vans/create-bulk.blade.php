...
@section('title')
{!! trans('system.action.import') !!} - Lương khoán
@stop
@section('head')
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <p class="help-block">Tải file theo định dạng sau <a href="{!! route('admin.vans.download') . '?time=' . time() !!}">tại đây</a></p>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <label>{!! trans('timekeeping.title') !!} <span class="text-danger">(*)</span></label>
                                    {!! Form::text('title', old('title',$title), ['class' => 'form-control title','id'=>'title' ]) !!}
                                    <span class="text-danger">
...
<script>
    ...
    function upload() {
        ...
        $('.loading').show();
        $.ajax({
            url: "{!! route('admin.vans.read-bulk') . '?time=' . time() !!}",
            data: myfile,
            type: 'POST',
            contentType: false,
            processData: false,
    ...
    function save() {
        ...
        $('.loading').show();
        $.ajax({
            url: "{!! route('admin.vans.save-bulk') !!}",
            data: { data: data, 'month': month, 'year': year,'company_id': company_id,'title': title },
            type: 'POST',
            datatype: 'json',
            headers: {'X-CSRF-Token': "{!! csrf_token() !!}"},
            success: function(res) {
                window.location.href = "{!! route('admin.vans.index') !!}";
            },
            error: function(obj, status, err) {
                var error = $.parseJSON(obj.responseText);
                toastr.error(error.message, '{!! trans('system.have_an_error') !!}');
