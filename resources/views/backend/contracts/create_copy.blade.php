    ...
    <div class="container col-md-12 contract-content">
        <div class="row">
            ...
            <div class="col col-md-4">
                {!! Form::select('user_id', ['' => trans('system.dropdown_choice')], old('user_id', $contract->user_id), ['id' =>'userSelect', 'class' => 'form-control select2 staffSelect', 'required']) !!}
            ...
            <div class="col col-md-4">
                {!! Form::select('company_id', ['' => trans('system.dropdown_choice')] + \App\Helpers\GetOption::getCompaniesForOption(),old('company_id', $contract->company_id), ['class' => 'form-control select2 companySelect', 'required']) !!}
            </div>
        </div>
        <div class="row">
            ...
            <div class="col col-md-4" id="department-tooltip" data-toggle="tooltip" data-placement="bottom" title="">
                {!! Form::select('department_id', ['' => trans('system.dropdown_choice')], old('department_id', $contract->department_id), ['disabled' => true, 'id' => 'departmentSelect', 'class' => 'form-control select2', 'required']) !!}
            </div>
            <label class="col col-md-2">{!! trans('contracts.position_id') !!}</label>
            <div class="col col-md-4">
                {!! Form::select('position_id', ['' => trans('system.dropdown_choice')] + \App\Helpers\GetOption::getStaffPositionsForOption(), old('position_id', $contract->position_id), ['class' => 'form-control select2 positionSelect', 'required']) !!}
            </div>
        </div>
        <div class="row">
            <label class="col col-md-2">{!! trans('contracts.title_id') !!}</label>
            <div class="col col-md-4">
                {!! Form::select('qualification_id', ['' => trans('system.dropdown_choice')] + \App\Helpers\GetOption::getStaffTitlesForOption(), old('qualification_id', $contract->qualification_id), ['class' => 'form-control select2', 'required']) !!}
            </div>
            <label class="col col-md-2">{!! trans('contracts.desc_qualification') !!}</label>
            <div class="col-md-4">
                {!! Form::textarea('desc_qualification', old('desc_qualification', $contract->desc_qualification), ['class' => 'form-control desc_qualification', 'rows' => 1]) !!}
            </div>
        </div>
        <div class="row">
        ...
        <div class="row">
            ...
            <div class="col col-md-4">
                ...
                <div class="input-group" style="display: inline-flex; width: 100%">
                    ...
                    <div style="width: 100%">
                        {!! Form::text('basic_salary', old('basic_salary', intval($contract->basic_salary) > 0 ? $contract->basic_salary : ''), ["class" => "form-control currency", 'autocomplete' => 'off',]) !!}
                    </div>
                </div>
            </div>
            ...
            <div class="col col-md-4">
                <div class="input-group">
                    ...
                    {!! Form::text('valid_from', Request::input('valid_from', strtotime($contract->valid_from) > 0 ?  date('d/m/Y', strtotime($contract->valid_from)) : ""), ['class' => 'form-control datepicker datepicker-from', 'required', 'autocomplete' => 'off']) !!}
                </div>
            </div>
        </div>
        <div class="row">
            ...
            <div class="col col-md-4">
                {!! Form::text('code', old('code', $contract->code), ['class' => 'form-control', 'maxlength' => 50,  ]) !!}
            </div>
            ...
            <div class="col col-md-4">
                <div class="input-group">
                    ...
                    {!! Form::text('valid_to1', Request::input('valid_to1', strtotime($contract->valid_to) > 0 ?  date('d/m/Y', strtotime($contract->valid_to)) : ""), ['class' => 'form-control datepicker datepicker-to', 'autocomplete' => 'off', 'disabled']) !!}
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <table class="table table-bordered expense">
                                ...
                                <tbody>
								<?php $allowance_cat = old('allowance_cat', []); $totalAmount = $totalAdvanced = 0; $allTc = 0 ?>
                                @if (count($allowance_cat))
                                    ...
                                    @endfor
                                @else
                                    @if (count($contract->allowances) == 0)
                                        <tr>
                                    ...
                                    @else
                                    {{-- @dd($contract->allowances) --}}
                                    @php
                                        $i = 1;
                                    @endphp
                                        @foreach ($contract->allowances as $item)
                                            <tr>
                                                <td style="text-align: center; vertical-align: middle;">{!! $i !!}</td>
                                                <td style="text-align: center; vertical-align: middle;">
                                                    {!! Form::select('allowance_cat[]', ['' => trans('system.dropdown_choice')] + $allowancesOption, old('allowance_cat[]', $item->category_id), ['class' => 'form-control select2 appendix_allowance_cat_new', ]) !!}
                                                </td>
                                                <td style="text-align: center; vertical-align: middle;">
                                                    {!! Form::text('allowance_cost[]', old('allowance_cost[]', $item->expense), ['class' => 'form-control currency appendix_allowance_cost_new allowance_cost']) !!}
                                                </td>
                                                <td style="vertical-align: middle;">
                                                    {!! Form::text("desc[]", old("desc[]", $item->desc), ['class' => 'form-control']) !!}
                                                </td>
                                                <td style="text-align: center; vertical-align: middle;">
                                                    @if ($i == 1)
                                                        <a href="javascript:void(0);" class="btn btn-xs btn-default add-expense">
                ...
                <div class="box-footer">
                    <table class="table table-total" style="width: 40%">
                        <tr style="font-size: 17px;">
                            ...
                            <th class="total-allowance-cost text-right">{!! \App\Helper\HString::currencyFormat($allTc) !!}</th>
                        </tr>
                        <tr style="font-size: 17px;">
                            <th style="width: 5%"></th>
                            <td style="text-align: left; font-weight: 500;">Tổng tiền lương + phụ cấp:&nbsp;&nbsp;&nbsp; </td>
                            <th class="total-amount text-right">{!! \App\Helper\HString::currencyFormat(intval($contract->basic_salary) + $allTc) !!}</th>
                        </tr>
                    </table>
                </div>
            </div>
        ...
        !function ($) {
            $(function () {
                ...
                if (contract != '') {
                    ...
                    $('#userSelect').append('<option selected value=' + user.id + '>' + user.code + " - " +  user.fullname + '</option>')
                    //userFromCreatStaff = user;
                    sessionStorage.setItem('codeUser', user.code);
                } 
                ...
                var counterExpense = $('select[name="allowance_cat[]"]').length + 1;
                ...
                var oldDepartmentId = {!! old('department_id') ?? $contract->department_id !!};
