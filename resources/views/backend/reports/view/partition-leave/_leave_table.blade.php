<table class="table table-bordered" style="width: 100%">
    ...
    $start = '26'; $end = '25';
    $style1 = "text-align: center; vertical-align: middle; background: {$bgColor[0]}; font-weight: 700; border: 1px solid black;";
            ...
            @foreach($comData as $departmentId => $deptData)
                ...
                @foreach($deptData as $userId => $item)
                    <tr>
                        ...
                        <td style="vertical-align: middle;text-align: left; border: 1px solid black;">{!! $item->fullname !!}</td>
                        <td style="vertical-align: middle; border: 1px solid black;">{!!$deptName !!}</td>
                        <td style="vertical-align: middle; border: 1px solid black;">{!! $companyName !!}</td>
                        <td style="vertical-align: middle; border: 1px solid black;">{!! $item->staff_start ? date('d/m/Y', strtotime($item->staff_start)) : '' !!}</td>
                        <td style="text-align: center; vertical-align: middle; border: 1px solid black;">{{  $item->original_rest }}</td>
                        <td style="text-align: center; vertical-align: middle; border: 1px solid black;">{{ $item->original_rest - $item->rest }}</td>
