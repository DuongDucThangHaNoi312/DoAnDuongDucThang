...
@section('title')
{!! trans('system.action.import') !!} - phân bổ bảo hiểm
@stop
@section('head')
...
@section('content')
    ...
    <section class="content-header">
        <h1>
            Nhập phân bổ chi phí lương theo xe
            <small>{!! trans('system.action.import') !!}</small>
        </h1>
        <ol class="breadcrumb">
            <li><a href="{!! route('admin.home') !!}">{!! trans('system.home') !!}</a></li>
            <li><a href="{!! route('admin.insurances.index') !!}">phân bổ bảo hiểm</a></li>
        </ol>
    </section>
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <p class="help-block">Tải file theo định dạng sau <a href="{!! route('admin.insurances.download') . '?time=' . time() !!}">tại đây</a></p>
                        ...
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="title" id="" class="form-control title" value="Phân bổ chi phí lương theo xe ">
                                </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <a href="{{ route('admin.insurances.index') }}" class="btn btn-danger btn-flat" >Hủy bỏ</a>
                        </div>
...
<script>
    ...
    function upload() {
        ...
        $.ajax({
            url: "{!! route('admin.insurances.read-bulk') . '?time=' . time() !!}",
            data: myfile,
            type: 'POST',
            contentType: false,
            processData: false,
    ...
    function save() {
        ...
        $.ajax({
            url: "{!! route('admin.insurances.save-bulk') !!}",
            data: { data: data, 'month': month, 'year': year, 'title': title, 'company_id':company_id },
            type: 'POST',
            datatype: 'json',
            headers: {'X-CSRF-Token': "{!! csrf_token() !!}"},
            success: function(res) {
                window.location.href = "{!! route('admin.insurances.index') !!}";
            },
            error: function(obj, status, err) {
                var error = $.parseJSON(obj.responseText);
                toastr.error(error.message, '{!! trans('system.have_an_error') !!}');
