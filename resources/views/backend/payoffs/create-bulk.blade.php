...
@section('title')
{!! trans('system.action.import') !!} - Các khoản tăng
@stop
@section('head')
...
@section('content')
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="{!! route('admin.vans.index') !!}">Các khoản tăng</a></li>
        </ol>
    </section>
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <p class="help-block">Tải file theo định dạng sau <a href="{!! route('admin.payoffs.download') . '?time=' . time() !!}">tại đây</a></p>
...
<script>
    ...
    function upload() {
        ...
        $('.loading').show();
        $.ajax({
            url: "{!! route('admin.payoffs.read-bulk') . '?time=' . time() !!}",
            data: myfile,
            type: 'POST',
            contentType: false,
            processData: false,
    ...
    function save() {
        ...
        var arr = [];
        if (!company_id) {
            arr.push('Công ty không được để trống');
        }
        if (!month) {
            arr.push('Tháng không được để trống');
        }

        if (!year) {
            arr.push('Năm không được để trống');
        }
        if (arr.length > 0) {
        ...
        $.ajax({
            url: "{!! route('admin.payoffs.save-bulk') !!}",
            data: { data: data, 'month': month, 'year': year,'company_id': company_id },
            type: 'POST',
            datatype: 'json',
            headers: {'X-CSRF-Token': "{!! csrf_token() !!}"},
            success: function(res) {
                window.location.href = "{!! route('admin.payoffs.index') !!}";
            },
            error: function(obj, status, err) {
                var error = $.parseJSON(obj.responseText);
                toastr.error(error.message, '{!! trans('system.have_an_error') !!}');
