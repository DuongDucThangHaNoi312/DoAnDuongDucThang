    ...
    <div style="width: 700px; margin: auto; margin-top: 50px;">
        ...
        <div class="box-body">
            ...
            <div  style="margin-top: 10px">
                ...
                {!! Form::text('name_es', old('name_es', $allowanceCategory->name_es), ['class' => 'form-control']) !!}
            </div>
            <div style="margin-top: 10px">
                {!! Form::label(trans('system.desc') ) !!}
                {!! Form::textarea('desc', old('desc', $allowanceCategory->desc), ['rows' => 4, 'class' => 'form-control']) !!}
            </div>
            <div style="margin-top: 10px">
                ...
                <select name="company_id" id="company" class="companySelect form-control select2">
                    ...
                    <option value="{{ $key }}" {{ $key == $allowanceCategory->company_id ? "selected" : ''}}>{{ $item }}</option>
                    @endforeach
                </select>
            </div>
            <div style="margin-top: 10px">
            ...
            <div align="center" style="margin-top: 30px; font-weight: 600;">
                ...
                <span style="margin-left: 10px">
                     {!! Form::checkbox('type', 1, old('type', $allowanceCategory->type), [ 'class' => 'minimal-red ' ]) !!}
                    {!! trans('allowance_categories.has_kpi') !!}
                </span>
                <span style="margin-left: 10px">
                    {!! Form::checkbox('is_social_security', 1, old('is_social_security', $allowanceCategory->is_social_security), [ 'class' => 'minimal-red ' ]) !!}
                    {!! trans('allowance_categories.is_social_security') !!}
                </span>
                <span style="margin-left: 10px">
                    {!! Form::checkbox('is_exemp', 1, old('is_exemp', $allowanceCategory->is_exemp), [ 'class' => 'minimal-red ' ]) !!}
                    {!! trans('allowance_categories.is_exemp') !!}
                </span>
                <span style="margin-left: 10px">
                    {!! Form::checkbox('ot', 1, old('ot', $allowanceCategory->ot), [ 'class' => 'minimal-red ' ]) !!}
...
@section('footer')
    ...
    <script>
        var db_department = '{{ $allowanceCategory->department }}';
        var ar = db_department.split(', ');
        !function ($) {
            $(".select2").select2({
                    width: '100%',
            });
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        $.each(res, function (index, value) {
                            let isSelected = ar.includes(index) ? 'selected' : '';
                            $('#departmentSelect').append('<option data-index='+ index +' value="' + index + '"' + isSelected + '>' + value + '</option>')
                        })
                    },
                    error: function (data) {
                        console.log(data)
