...
@section('content')
    <section class="content-header">
        <h1>
            Thêm mới
            <small>nhóm nhân viên - {{ $department->name }}</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content overlay">
        <div class="box">
            <div class="box-body no-padding">
                {!! Form::open([ 'url' => route('admin.departments.store-team'), 'method' => 'POST']) !!}
                                        ...
                                        @foreach ($users as $item)
                                            <tr>
                                                <td style="text-align: center">
                                                    <input class="checkbox-{{ $item->id }}" type="checkbox" name="user_id[]" id="" value="{{ $item->id }}">
                                                </td>
                                                <td>{{ $item->fullname . ' - ' . $item->code }}</td>
                                                <td style="text-align: center">
                                                    <input data-id="{{ $item->id }}" type="radio" name="lead_user_id" id="" value="{{ $item->id }}" required>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                    ...
                    <div class="row">
                        <div class="col-md-8" style="text-align: center">
                            <a href="{{ route('admin.departments.list-team', $department->id) }}" class="btn btn-danger btn-sm">Trở lại</a>
                            @if (count($users) > 0)
...
@section('footer')
    ...
    <script>
        $(function() {
            ...
            $(".userSelect").select2({
                ajax: {
                    url: '{!! route("admin.team.searchUser", $department->id) !!}',
                    dataType: 'json',
                    delay: 250,
