...
@section('content')
    ...
    {!! Form::open(['url' => route('admin.combined.update',$departmentGroup->id), 'method'=>'PUT']) !!}
    ...
    <table class='table borderless' style="width: 1000px;margin: auto;margin-top: 50px;">
        <tr>
            ...
            <td>
                {!! Form::text('name', old('name',$departmentGroup->name), ['class' => 'form-control']) !!}
            </td>
            <th class="table_right_middle " style="width: 15%;">
            </th>
            <td>
            ...
            <td>
                <select class="form-control select2" multiple name="department_id[]" >
                    @foreach($departmentOptions as $key => $value)
                        <option value="{!! $key !!}" @if(in_array($key, $departmentId)) selected @endif>{!! $value !!}</option>
                    @endforeach
                </select>
            </td>
        </tr>
        <tr >
            ...
            <td>
                {!! Form::select('type', ['' => trans('system.dropdown_choice')]+\App\Define\Department::getTypeDepartmentGroups() ,old('type',$departmentGroup->type), ['class' => 'form-control select2 company-id', ]) !!}
            </td>
        </tr>
        <tr>
            <td class="text-center table_right_middle1" colspan="3">
                <label>
                    {!! Form::checkbox('only_manager', 1, old('only_manager',$departmentGroup->only_manager), [ 'class' => 'minimal' ]) !!}
                    {!! trans('departments.combined_only_manager') !!}
                </label>
                &nbsp; &nbsp; &nbsp; &nbsp;
                <label>
                    {!! Form::checkbox('status', 1, old('status',$departmentGroup->status), [ 'class' => 'minimal' ]) !!}
